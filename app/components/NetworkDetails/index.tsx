...
const NetworkDetails = ({ stats }: { stats: NetworkStatus }) => {
  ...
  return (
    <div className="relative isolate overflow-hidden bg-rose-50 py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <dl className="grid grid-cols-1 gap-x-8 gap-y-16 text-center lg:grid-cols-3">
          {dataSet.map((set) => (
            <div key={set.title} className="mx-auto flex max-w-xs flex-col gap-y-4">
              <dt className="text-base leading-7 text-fuchsia-950">{set.title}</dt>
              <dd className="order-first text-3xl font-semibold tracking-tight text-fuchsia-900 sm:text-5xl">
                {set.value}
              </dd>
            </div>
          ))}
